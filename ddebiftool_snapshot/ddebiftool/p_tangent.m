...
is_ext=dde_method_check(mth,'extracolumns','auto')||...
    dde_method_check(mth,'extracolumns',@(n)~isempty(n)&&n>0);
if all(size(border)==[size(J,2),nulldim]) || ...
        (size(border,2)==nulldim && size(border,1)<=size(J,2) && is_ext)
    rhs=cat(1,zeros(size(J,1),nulldim),eye(nulldim));
    border=cat(1,border,zeros(size(J,2)-size(border,1),nulldim));
