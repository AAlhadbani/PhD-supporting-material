...
\item Discrete symmetries and tracking or suppressing of symmetry
  ...
  \blist{pendulum} (\file{balancing\_demo.m}) shows tracking of
...
\item Added demos:
  \begin{itemize}
  \item \blist{pendulum} (\file{balancing\_demo.m}, a
...
web addresses
\begin{tabbing}
  ver.~$\geq3.0$\qquad \= \ddebifarx{}\\
  ver.~$\leq2.03$ \> \ddebifwebold{}.
\end{tabbing}
...
\begin{tabbing}
  ...
  using FORTRAN \cite{Doed07,Doed99},\\
...
into the user's path. The folder \url{../demos/Mackey-Glass} and other demo folders contains a function \blist{ddebiftoool_path}, which makes it easy to load all paths by pointing to the base installation folder. It is recommended to copy the function file \file{ddebiftool\_path.m} into the respective current working folder whenever one starts exploring a problem.
\begin{lstlisting}
...
{\DDEBIFCODE} right-hand side, $f(x,p)$, where the state variables
...
$n_\mathrm{vec}$, to result in an array of shape $1\times n_\mathrm{vec}$ with the values of
...
Some information about the points such as \texttt{stability} has not yet been
...
containing an array of (at least two) points, four sets of method
...
\item Torus bifurcation (\blist{TorusBifurcation}): complex pair of
...
$x^*_0=x^*(t)$, $\tau_j^*(t)=\tau_j(x^*_0,\ldots,x^*_{n_{\tau,j}},p^*)$ and
...
\begin{aligned}
  A_j(t)&=\frac{\partial f}{\partial x^j}(x^*_0,x^*_1,\ldots,x^*_{n_\tau},p^*)\in\RR^{n_f\times n_x}
  \mbox{,\quad ($j=0,\ldots,m$),}\\
  B_{j,k}(t)&=\frac{\partial \tau_j}{\partial x^k}(x^*_0,x^*_1,\ldots,x^*_{n_{\tau,j}},p^*)\in\RR^{1\times n_x}\mbox{,\quad ($k=0,\ldots,j-1$, $j=1,\ldots,n_\tau$).} 
\end{aligned}
\end{equation}
...
$y_{0,j}$ are selected as index sets from the parameter vector $p$
...
constants, while the upper bound  $\tau_{\max}$ of the delay chain and
...
\item \label{defsys:num} For initial exploration of equilibria, periodic orbits and
  ...
  the right-hand sides, as described in
...
\item If problems or inexplicable discrepancies between results using
  ...
  delays).
\item For backward compatibility the user may also provide first-order derivatives in the ``\blist{sys_deri}'' format compatible with versions v.~3.1
  and earlier, explained in \Cref{sec:constjac} for constant delays
  and \Cref{sec:sdjac} for state-dependent delays. This is not
  recommended and will be wrapped and converted to directional
  derivatives internally up to first order. The second derivatives are
  no longer directly used. Only the \blist{hcli} code uses the
  \blist{sys_deri} format.
\end{enumerate}
...
For manually implemented derivatives, the user should provide
...
\item $\blist{par}\in\RR^{1\times
  p}$ contains the parameters, $\blist{par}=p$.
\end{itemize}
The delays $\tau_i$ ($i=1\ldots,n_\tau$) are considered to be part of the
parameters ($\tau_i=p_{j(i)}$, $i=1,\ldots,n_\tau$).  This is natural
...
\end{lstlisting}
where \blist{f}$\in\mathbb{R}^{n_x}$ is a symbolic expression containing
symbols from the arrays of symbols \blist{x}$\in\mathbb{R}^{n_x\times(n_\tau+1)}$
and \blist{par}\,$\in\mathbb{R}^{n_p}$. The arrays have to be of a form such
...
\blist{fname} is a valid file name (without suffix). The output is then stored in the current folder as the
...
\item \blist{'sd_delay'}: $n_\tau\times1$ symbol array of expressions for
...
\item \blist{'multifile'} (logical, default \blist{false}) store each
  ...
  subfunctions (which may have the same names in several
...
the right-hand side $f(x_0,\ldots,x_{n_\tau},p)$,
\begin{displaymath}
  \d^kf(x_0,\ldots,x_{n_\tau},p)[\tilde{x}_0,\ldots,\tilde{x}_{n_\tau},\tilde{p}]^k:=
  \frac{\d^k}{\d h^k}f(x_0+h\tilde{x}_0,\ldots,x_{n_\tau}+h\tilde{x}_{n_\tau},p+h\tilde{p})\vert_{h=0}\mbox{,}
\end{displaymath}
...
Arguments:
\begin{itemize}
\item $\blist{xx}\in\RR^{n_x\times (n_\tau+1)\times n_\mathrm{vec}}$ contains the state
  variable(s), $\blist{xx(:,j,:)}=x_{j-1}$, $x_0,\ldots,x_{n_\tau}$, at the present and in the past (as
  for the right-hand side),;
\item $\blist{par}\in\RR^{1\times
  n_p\times n_\mathrm{vec}}$ contains the parameters, $\blist{par}=p$  (as for the right-hand side);
\item \blist{dxx}$\in\RR^{n_x\times(n_\tau+1)\times n_\mathrm{vec}}$ contains the directions of the derivative from
  \blist{xx}: \blist{dxx(:,j,:)}$=\tilde{x}_{j-1}$ for
    $j=1\ldots,n_\tau+1$.
\item \blist{dpar}$\in\RR^{1\times
  n_p}$ contains the directions of the derivative in the parameters, $\blist{dpar}=\tilde{p}$.
\end{itemize}
...
$5$ are used. Any derivatives that are not provided by  \blist{sys_dirderi} (or \blist{sys_deri}) will be approximated by finite
...
\begin{quote}
  ...
  derivatives.
\end{quote}
The old format of specifying the Jacobians is to provide a function
with a header of the format
\begin{lstlisting}
  function J=sys_deri(xx,par,ix,ip,v)
\end{lstlisting}
Arguments:
\begin{itemize}
\item $\blist{xx}\in\RR^{n_x\times (n_\tau+1)}$ contains the state
variable(s) at the present and in the past (as for the right-hand side);
\item $\blist{par}\in\RR^{1\times n_p}$ contains the parameters, $\blist{par}=p$  (as for the right-hand side);
\item \blist{ix} (empty, one integer or two integers) index (indices) of
  \blist{xx} with respect to which the right-hand side is to be
  differentiated
\item \blist{ip} (empty or integer) whether right-hand side is to be
  differentiated with respect to parameters
\item \blist{v} (empty or $\CC^{n_x}$) for mixed derivatives with respect
...
requested via \blist{ix} and \blist{ip} multiplied with \blist{v} (when
nonempty), see table \ref{deri_requested}.
\begin{table}[htbp]
\begin{center}
\begin{tabular}{ccc@{\hspace*{5em}}l}
\noalign{\medskip}\hline\noalign{\smallskip}\blist{length(ix)} & \blist{length(ip)}  & \blist{v} & \blist{J} 
\\\noalign{\smallskip}\hline\noalign{\medskip}
1         & 0         & empty      & 
$\frac{\textstyle\partial f}{\textstyle\partial x_{\blist{ix(1)}}}
=A_{\blist{ix(1)}}\in\RR^{n_f\times n_x}$ \\[3ex]
0         & 1         & empty      & 
$\frac{\textstyle\partial f}{\textstyle\partial p_{\blist{ip(1)}}}
\in\RR^{n_f\times 1}$ \\[3ex]
1         & 1         & empty      & 
$\frac{\textstyle\partial^2 f}{\textstyle\partial x_{\blist{ix(1)}}\partial p_{\blist{ip(1)}}}
\in\RR^{n_f\times n_x}$ \\
2         & 0         & $\in\CC^{n_x\times1}$ & 
$\frac{\textstyle\partial}{\textstyle\partial x_{\blist{ix(2)}}}
\left(A_{\blist{ix(1)}}v\right)
\in\CC^{n_f\times n_x}$\\\noalign{\medskip}\hline
\end{tabular}
\caption{\label{deri_requested}
Results of the function \blist{sys\_deri} depending on its
input parameters \blist{ix}, \blist{ip} and \blist{v}
using $f\equiv f(x_0,x_1,\ldots,x_{n_\tau},p)$.}
\end{center}
\end{table}
...
\blist{ix} is a number between $0$ and $n_\tau$ based on $f\equiv
f(x_0,x_1,\ldots,x_{n_\tau},p)$.  E.g., if \blist{ix} has only one element
take the derivative with respect to $x_{\blist{ix(1)}}$.  If it has
two elements, take, of the result, the derivative with respect to
$x_{\blist{ix(2)}}$ and so on.  Similarly, if \blist{ip} is nonempty
take, of the resulting \blist{J}, the derivative with respect to
$p_{\blist{ip(i)}}$ where $i$ ranges over all the elements of
\blist{ip}, $1\leq i \leq n_p$.  Finally, if $v$ is not an empty vector
...
are taken (when \blist{ix} contains two elements).  Not all possible
combinations of these derivatives have to be provided.  In the current
version, \blist{nx} has at most two elements and \blist{ip} at most
...
are given by,
\[
\blist{J}_{i,j}=\left[\frac{\partial}{\partial x_{\blist{ix(2)}}}
A_{\blist{ix(1)}}v\right]_{i,j}
=\frac{\partial}{\partial x_{\blist{nx(2)},j}}
\left(\sum_{k=1}^{n_x}\frac{\partial f_i}{\partial x_{\blist{ix(1)},k}} v_k
\right),
\]
...
determining systems for fold, Hopf, period-doubling and torus bifurcations
%G.'s add
...
As an illustrative example we will use the following system of delay 
differential equations (see \url{../demos/sd_demo}),
...
past, $\blist{xx}=[x(t), x(t-\tau_1), \ldots,x(t-\tau_{n_\tau})]$.  In
this case the constant delays ($\tau_1$ and $\tau_2$ in example
\eqref{example_sys2}) are also part of the parameters (not necessary for sd-DDEs if the delays are not independently varied, e.g. for distributed delays). See Listing
\ref{sd_rhs} for the right-hand side to be provided for example
\eqref{example_sys2}. Vectorization will speed up computation for
periodic orbits.  \lstinputlisting[frame=lines,float,label=sd_rhs,
basicstyle={\ttfamily\small},caption={Listing of right-hand side
  \blist{'sys_rhs'} function for \eqref{example_sys2}, here called
  \blist{sd_rhs} (not vectorized).}]{sd_rhs.m}
...
\blist{sys_ntau} has no arguments and returns the number of delays. For the example \eqref{example_sys2},
...
\begin{lstlisting}
  for i=1:ntau
    tau(i)=funcs.sys_tau(i,x(:,1:i),p);
    % get x(t-tau(i)), insert into array x(1:nx,1:ntau)...
   end
\end{lstlisting}
Here, at step $i$, $x$ has shape $n_x\times i$. If the system has
many delays (e.g, when approximating distributed delays), the provision
...
delay is passed on as name-value pair: \blist{dde_sym2funcs(...,'sd_delay',tau,...)}. If not provided by the user, the argument \blist{'sys_tau_seq'} is deduced automatically from the expressions in the arguments \blist{delays}, \blist{x} and \blist{par}.
...
derivative of the delay function $\tau_j(x_0,\ldots,x_{j-1},{p})$,
\begin{displaymath}
  \d^k\tau_j(x_0,\ldots,x_{j-1},{p})[\tilde{x}_0,\ldots,\tilde{x}_{j-1},\tilde{{p}}]^k:=
  \frac{\d^k}{\d h^k}\tau_j(x_0+h\tilde{x}_0,\ldots,x_{j-1}+h\tilde{x}_{j-1},{p}+h\tilde{{p}})\vert_{h=0}\mbox{,}
\end{displaymath}
 has to have the form
\begin{lstlisting}
...
\item \blist{xx} ($n_x\times\blist{ind}$ matrix) is the state:
  \blist{xx(:,1)}$=x(t)$, \blist{xx(:,i)}$=x(t-\tau_{i-1})$ for
  $i=2\ldots\blist{ind}$;
\item \blist{par}$\in\RR^{1\times n_p}$ are the parameters (as for the right-hand side)
\item \blist{dxx} ($n\times\blist{ind}$ matrix) are the deviations $\tilde{x}_i$ from \blist{xx}
\item \blist{dpar}$\in\RR^{1\times
  p}$ contains the deviations to the parameters, $\blist{dpar}=\tilde{{p}}$.
\end{compactitem}
...
Directional derivatives should preferably be provided in vectorized form.
...
order directional derivatives of \blist{sys_rhs} in \blist{sys_dirderi}.
...
%     \partial^{\phantom{l}} {p}_{\blist{np(1)}}} \in\RR$ \\[3ex]
%   1         & 1         & 
%   $\frac{\textstyle\partial^2 \tau_{\blist{ind}\phantom{y}}}{\textstyle
%     \partial x^{\blist{nx(1)}}\ \partial {p}_{\blist{np(1)}}}
%   \in\RR^{n}$ \\[3ex]
%   2         & 0         & 
...
\begin{quote}
  ...
  automatically generated from symbolic expressions into structure.
\end{quote}
...
follows (see \url{../demos/neuron/demo1_funcs.m}):
\begin{lstlisting}
funcs=set_funcs('sys_rhs',neuron_sys_rhs,'sys_tau',@()[5,6,7],...
                'sys_dirderi',neuron_sys_dirderi);
\end{lstlisting}
Note that \blist{neuron_sys_rhs} and \blist{neuron_sys_dirderi} are variables (function handles
pointing to an anonymous function defined as in
section~\ref{sec:constrhs}). The delay function
...
analytical derivatives, one may drop the \blist{'sys_dirderi'} pair (then
...
\item \blist{'sys_deri'} (not recommended): handle of the user function providing the
...
\item \blist{'sys_dtau'} (not recommended, relevant for sd-DDEs only): handle of the user
...
\item \blist{'lhs_matrix'} (numerical, default identity): the
  $n_f\times n_x$ matrix $M$ on the left-hand side of the differential equation;
\item \blist{'sys_cond'} (default empty): handle or struct for user-specified extra conditions
  (see \cref{sec:syscond} and \cref{extra_cond})
\item \blist{'sys_cond_reference'}: (logical, default \blist{false}):
  flag indicating if \blist{'sys_cond'} takes extra argument; is ignored if \blist{'sys_cond'} is struct, see \cref{sec:syscond};
...
sets $i$ (\blist{'x'} or \blist{'ix'}), $k$ (\blist{'value'}) and $\ell$ (\blist{'par'} or \blist{'ipar'}) to \blist{dde_add_dist_delay}.
\begin{lstlisting}
g=@(s,x,p)x.*(1-x);
...
distributed delays permits renewal equations and  non-zero lower
...
\item $s_\mathrm{d,A}(t)$, the resource infulence on individual growth to maturity.
\end{compactitem}
...
\begin{align*}
  ...
  \mbox{for\ }a\in[0,a_{\max}]&&  y_{\mathrm{id},2}(a,t)&=g_\mathrm{deff}(a,y_2(a,t),q_2)&&\mbox{for $y_2(a,t)=(y_{\mathrm{int},1}(a,t),y_{\mathrm{id,0,2}}(a,t))$}\\
...
$q_1=(p_3,p_4)$, $q_2=(p_1,p_4,p_5,p_7,p_8)$.  As one can see, the chain of
...
components of previously defined $y_{\mathrm{id},\ell}(a,t)$ and
$y_{\mathrm{int},\ell}(a,t)$. Then $y_{\mathrm{int},j}(a,t)$ is the
...
$n_\mathrm{d}$ distributed delays by combining $n_\mathrm{c}$ selected components of
the $y_{\mathrm{id},k}(a,t)$ and $y_{\mathrm{int},k}(a,t)$ at
$n_\mathrm{s}$ user-selected $r_k\tau_{\max}$ ($k=1,\ldots,n_\mathrm{s}$):
\begin{align}\label{dist:chain:sum}
  x_k&=\sum_{k=1}^{n_\mathrm{s}}S_{\mathrm{s},k}y_\mathrm{s}(r_{\mathrm{s},k}a_{\max},t).
\end{align}
Here $y_\mathrm{s}(r_k\tau_{\max},t)$ is a subvector of the sequence
of $y_{\mathrm{id},k}(a,t)$ and $y_{\mathrm{int},k}(a,t)$ of length $n_c$. The coefficients are $n_\mathrm{s}$ matrices $S_{\mathrm{s},k}$ ($k=1,\ldots,n_\mathrm{s}$) of shape $n_\mathrm{d}\times n_\mathrm{c}$.  
...
$s_\mathrm{d}$ and $d_\mathrm{eff}$. Thus, $n_\mathrm{c}=2$. 
\item The argument \blist{'value'}
lists the $n_\mathrm{d}$ variable indices to which the distributed delays are
assigned. In the example $n_\mathrm{d}=3$.
\item The argument \blist{'it'} are the $n_\mathrm{s}$ fractions $r_{\mathrm{s},k}$
  at which the selected chain elements $y_{\mathrm{s},k}$ are evaluated. In the example $n_\mathrm{s}=2$.
\item The argument \blist{'t_type'} flags if the indices in \blist{'it'} are parameter or states. In the example, the
\item The argument \blist{'S'} is a cell array of length $n_d$ of the matrices
$S_{\mathrm{s},k}$ (in the example all $3\times 2$), defining the linear combinations resulting in the
distributed delays. 
\end{compactitem}
...
A connecting orbit is represented by the parameter values ${p}$, the
...
u\left(\left[c_{i,j}-\frac{\tau_m}{T}\right\vert_{\mod[0,1]}\right),{p}\right)\mbox{,}\\
