...
iccut=find(diff(sign(psol.profile(1,icoarse))));
icut=1+(iccut-1)*psol.degree;
hcli1=psol;
hcli1.mesh=hcli1.mesh(icut(1):icut(2));
hcli1.profile=hcli1.profile(:,icut(1):icut(2));
hcli1.period=hcli1.period*(hcli1.mesh(end)-hcli1.mesh(1));
hcli1.mesh=(hcli1.mesh-hcli1.mesh(1))/(hcli1.mesh(end)-hcli1.mesh(1));
%%
hcli1=p_tohcli(funcs,hcli1) % convert it to a point of homoclinic structure

mh=df_mthod(funcs,'hcli');
mh.point.print_residual_info=1
...
hcli2=psol;
hcli2.mesh=hcli2.mesh(81:end-4);
hcli2.profile=hcli2.profile(:,81:end-4);
