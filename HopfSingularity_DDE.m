...
% DDE examples, already linearized in assumed zero equilibrium
% 
% Example 1
%   y1'(t)=-y1(t-tau)
% bifurcation parameter a=tau, value tau0=a0=pi/2, om0=1
% Example 2
%   y1'(t)=-y2(t), y2'(t) = -c y(t) - d y(t-tau)
%
% bifurcation parameter a=tau, Hopf bifurcation at a0=pi/12, omega0=2,
% c=2/sqrt(3), d=8/sqrt(3)
%
%%
clear
...
switch example
    case '1d'
        n_x=1;
        [a0,om0]=deal(sym(pi)/2,sym(1));
        dfx=@(a,y)-y(-a);   % functional f_x(a)
        dgx=@(a,y)-0*y(0);  % functional g_x(a)
        sub0=@(ex)subs(ex,[om,a],[om0,a0]); % substitute bifurcation parameter values
    case '2d'
        n_x=2;
        dfx=@(a,y)[ind(y(0),2);-c*ind(y(0),2)-d*ind(y(-tau),1)];   % functional f_x(a)
        dgx=@(a,y)-0*y(0);  % functional g_x(a)
        a=tau; % bifurcation parameter
        % fixed parameters
        [     a0,       om0,   tau0,       c0,            d0]=deal(...
            sym(pi)/12, sym(2),sym(pi)/12,2/sqrt(sym(3)),8/sqrt(sym(3)));
        sub0=@(ex)subs(ex,[om,a,c,d],[om0,a0,c0,d0]); % substitute bifurcation parameter values
end
% characteristic matrix
Id=num2cell(sym(eye(n_x)),1);
Hv=@(v)lam*v+dgx(a,@(t)exp(lam*t)*v)-dfx(a,@(t)exp(lam*t)*v);
...
assume([zr,zi],'real')
assert(isAlways(all(0==simplify(H(-1i*om,a)*JCR([zr,zi])-JCR(Hr([zr,zi],om,a))),'All')));
...
Mbr=simplify(...
    [ipr(p1,Hr_om0),ipr(p1,Hr_a0);...
     ipr(p2,Hr_om0),ipr(p2,Hr_a0)])
%% reduced nonlinearity and its derivative at zero
m=3; %number of Fourier modes
